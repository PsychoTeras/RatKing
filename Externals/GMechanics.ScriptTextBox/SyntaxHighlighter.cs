...
namespace GMechanics.ScriptTextBox
{
    public class SyntaxHighlighter: IDisposable
    {
        ...
        public readonly Style BoldStyle = new TextStyle(null, null, FontStyle.Bold | FontStyle.Italic);
        ...
        public virtual void HighlightSyntax(Language language, Range range)
        {
            switch (language)
            {
                case Language.GameScript: GameScriptSyntaxHighlight(range); break;
        ...
        {
            ScriptTextBoxControl tb = (sender as ScriptTextBoxControl);
            Language language = tb.Language;
            switch (language)
            {
                case Language.GameScript: GameScriptAutoIndentNeeded(sender, args); break;
        ...
        private void GameScriptAutoIndentNeeded(object sender, AutoIndentEventArgs args)
        {
            //block {}
            if (Regex.IsMatch(args.LineText, @"^[^""']*\{.*\}[^""']*$"))
                return;
    ...
    brackets.Attributes["left2"].Value == "" || brackets.Attributes["right2"].Value == "")
        ...
        private Regex _gameScriptStringRegex,
            _gameScriptCommentRegex1,
            _gameScriptCommentRegex2,
            _gameScriptCommentRegex3,
            _gameScriptNumberRegex,
            _gameScriptAttributeRegex,
            _gameScriptClassNameRegex,
            _gameScriptKeywordRegex,
            _gameScriptGameObjectRegex;

        void InitGameScriptRegex()
        {
            _gameScriptStringRegex = new Regex(@"""""|@""""|''|@"".*?""|(?<!@)(?<range>"".*?[^\\]"")|'.*?[^\\]'", RegexCompiledOption);
            _gameScriptCommentRegex1 = new Regex(@"//.*$", RegexOptions.Multiline | RegexCompiledOption);
            _gameScriptCommentRegex2 = new Regex(@"(/\*.*?\*/)|(/\*.*)", RegexOptions.Singleline | RegexCompiledOption);
            _gameScriptCommentRegex3 = new Regex(@"(/\*.*?\*/)|(.*\*/)", RegexOptions.Singleline | RegexOptions.RightToLeft | RegexCompiledOption);
            _gameScriptNumberRegex = new Regex(@"\b\d+[\.]?\d*([eE]\-?\d+)?[lLdDfF]?\b|\b0x[a-fA-F\d]+\b", RegexCompiledOption);
            _gameScriptAttributeRegex = new Regex(@"^\s*(?<range>\[.+?\])\s*$", RegexOptions.Multiline | RegexCompiledOption);
            _gameScriptClassNameRegex = new Regex(@"\b(function)\s+(?<range>\w+?)\b", RegexCompiledOption);
            _gameScriptKeywordRegex = new Regex(@"\b(event|interrupt|break|case|subject|continue|default|else|false|for|foreach|function|global|if|lock|null|object|return|switch|true|while|var|yield)\b|#region\b|#endregion\b", RegexCompiledOption);
            _gameScriptGameObjectRegex = new Regex(@"@Object|@Subject|@Member|@Value|@OldValue|@Cancelled", RegexCompiledOption);
        }
        /// <summary>
        /// Highlights C# code
        /// </summary>
        /// <param name="range"></param>
        public virtual void GameScriptSyntaxHighlight(Range range)
        {
            ...
            range.ClearStyle(BlueStyle, BoldStyle, GrayStyle, MagentaStyle, 
                             GreenStyle, BrownStyle, DodgerBlueStyle);
            //
            if (_gameScriptStringRegex == null)
                InitGameScriptRegex();
            //string highlighting
            range.SetStyle(BrownStyle, _gameScriptStringRegex);
            //comment highlighting
            range.SetStyle(GreenStyle, _gameScriptCommentRegex1);
            range.SetStyle(GreenStyle, _gameScriptCommentRegex2);
            range.SetStyle(GreenStyle, _gameScriptCommentRegex3);
            //number highlighting
            range.SetStyle(MagentaStyle, _gameScriptNumberRegex);
            //attribute highlighting
            range.SetStyle(GrayStyle, _gameScriptAttributeRegex);
            //class name highlighting
            range.SetStyle(BoldStyle, _gameScriptClassNameRegex);
            //keyword highlighting
            range.SetStyle(BlueStyle, _gameScriptKeywordRegex);
            //game objects highlighting
            range.SetStyle(MagentaStyle, _gameScriptGameObjectRegex);
    ...
    {
        Custom, GameScript, CSharp, VB, HTML, SQL, PHP, JS
    }
}
